...
f = A*x + B*u   %state equation
% g = -(C*x+G*sqrt(1+C*u.^2))
g = -(x+(L.*sqrt(1+f.^2))) %(argumented)cost function / lagrangian

% euler lagrange equation
solve(jacobian(g,x) - jacobian(g,u) ==0, u2)

Ldot = - jacobian(f,x)'*L - jacobian(g,x)' %adjoint equation

eq1 = 0 == jacobian(f,u)'*L + jacobian(g,u)' %Control equation
